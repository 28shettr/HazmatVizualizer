<script lang="ts">
  ...
  function insertLineAfter(seqIndex: number) {
    ...
    if (!newPoint) {
      ...
      if (currentLine && currentLine.endPoint) {
        newPoint = {
          x: (currentLine.endPoint.x ?? 0) + _.random(-12, 12),
          y: (currentLine.endPoint.y ?? 0) + _.random(-12, 12),
          heading: "tangential",
          reverse: false,
        };
      } else {
        newPoint = {
          x: _.random(-70.75, 70.75),
          y: _.random(-70.75, 70.75),
          heading: "tangential",
          reverse: false,
        };
      }
  ...
  function addControlPointToLine(seqIndex: number) {
    ...
    const endPt = line.endPoint || { x: 0, y: 0 };
