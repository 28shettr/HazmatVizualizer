<script lang="ts">
  ...
  let rulerStart = { x: -50, y: 0 };
  let rulerEnd = { x: 10, y: 0 };
  let rulerDragging: "start" | "end" | null = null;

  let protractorPos = { x: 0, y: 0 };
  ...
  const FIELD_MIN = -FIELD_SIZE / 2;
  const FIELD_MAX = FIELD_SIZE / 2;
  let spacing = 12;
  ...
  $: gridPositions = (() => {
    ...
    // Anchor the grid at 0 so the axes line up with field-center coordinates,
    // and walk outward toward both edges.
    const start = Math.ceil(FIELD_MIN / spacing) * spacing;
    for (let pos = start; pos <= FIELD_MAX; pos += spacing) {
      positions.push(Number(pos.toFixed(6)));
    }
    if (positions[0] !== FIELD_MIN) {
      positions.unshift(FIELD_MIN);
    }
    if (positions[positions.length - 1] !== FIELD_MAX) {
      positions.push(FIELD_MAX);
    }
    return positions;
  })();
  ...
  <svg class="absolute top-0 left-0 w-full h-full pointer-events-none z-20">
    ...
    {#each gridPositions as position, i}
      <line
        ...
        opacity="0.3"
      />
      {#if i % labelInterval === 0 || position === FIELD_MIN || position === FIELD_MAX}
        <text
          x={x(position)}
          y={y(FIELD_MIN) + 15}
